...
export class CreditCommand extends Command {
	...
	public async exec(message: Message, args: any): Promise<any> {
		if (!DiscordUtil.hasAnyRole(message.member, this.config.get()["commands"]["credit"]["permissions"])) return message.channel.send("You do not have the required role to do that.");
		...
		if (args.subCommand === undefined || args.subCommand !== "add" && args.subCommand !== "remove") return message.channel.send("That is an invalid subcommand.");
		if (args.receiver === undefined) return message.channel.send("That is an invalid user.");
		if (args.amount === undefined || args.amount <= 0) return message.channel.send("That is an invalid amount.");
		let reply: string;
		args.subCommand === "add"
